...
// value: GetItemTypeId(item) or GetUnitTypeId(unit) or ability
function DzAPI_Map_Ladder_SubmitObjectData takes player whichPlayer, string key, integer value returns nothing
	if(value!=0)then
		call DzAPI_Map_Ladder_SetStat(whichPlayer,key,I2S(value))
	endif
endfunction
